...
import { getBoyfriendById } from "../api/api";
...
const BoyfriendProfile = () => {
    ...
    useEffect(() => {
        if (id === "preview") return; // Skip fetch for preview mode

        const fetchBoyfriend = async () => {
            try {
                const { data } = await getBoyfriendById(id);
                setBoyfriend(data);
                if (data.images && data.images.length > 0) {
                    setMainImage(data.images[0]);
                }
            } catch (err) {
                console.error("Failed to fetch boyfriend", err);
        ...
        <div className="container max-w-6xl py-8 px-4 md:px-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="space-y-8">
                    ...
                    {/* Reviews Section */}
                    {boyfriend.reviews && boyfriend.reviews.length > 0 ? (
                        <div className="pt-8 border-t">
                            <h3 className="text-2xl font-bold mb-6">Reviews</h3>
                            <div className="space-y-6">
                                {boyfriend.reviews.map((review, i) => (
